...
# os.environ['CAKE_DB_LOCATION'] = '/fdp-d3d/metadata/iri_logs.db'

from toksearch import MdsSignal, Pipeline  
#from toksearch_d3d import CakeSignal, PtDataSignal
import pdb
...
def create_efit_pipeline(shots):
	...
	sigs_dict["q"] = MdsSignal(r'\QPSI', efit_tree, dims=("psi","times"), data_order=("times","psi"))#, location=loc)
	sigs_dict["p"] = MdsSignal(r'\PRES', efit_tree, dims=("psi","times"), data_order=("times","psi"))#, location=loc)
	sigs_dict["RBBBS"] = MdsSignal(r'\TOP:RESULTS:GEQDSK:RBBBS', efit_tree, dims=("nbbbs", "times"), data_order=("times", "nbbbs"))#, location=loc)
	sigs_dict["ZBBBS"] = MdsSignal(r'\TOP:RESULTS:GEQDSK:ZBBBS', efit_tree, dims=("nbbbs", "times"), data_order=("times", "nbbbs"))#, location=loc)
	sigs_dict["aminor"] = MdsSignal(r'\TOP:RESULTS:AEQDSK:AMINOR', efit_tree)#,location=loc) # over time, doesn't need dims or data_order
	sigs_dict["bcentr"] = MdsSignal(r'\TOP:RESULTS:AEQDSK:BCENTR', efit_tree)#,location=loc) # over time, doesn't need dims or data_order
	sigs_dict["kappa"] = MdsSignal(r'\TOP:RESULTS:AEQDSK:KAPPA', efit_tree)#,location=loc) # over time, doesn't need dims or data_order
	sigs_dict["q0"] = MdsSignal(r'\TOP:RESULTS:AEQDSK:Q0', efit_tree)#, location=loc) 
	sigs_dict["q95"] = MdsSignal(r'\TOP:RESULTS:AEQDSK:q95', efit_tree)#, location=loc) # over time, doesn't need dims or data_order
	sigs_dict["li"] = MdsSignal(r'\TOP:RESULTS:AEQDSK:li', efit_tree)#, location=loc) # over time, doesn't need dims or data_order
	sigs_dict["rcur"] = MdsSignal(r'\TOP:RESULTS:AEQDSK:rcur', efit_tree)#, location=loc) # over time, doesn't need dims or data_order
	sigs_dict["zcur"] = MdsSignal(r'\TOP:RESULTS:AEQDSK:zcur', efit_tree)#, location=loc) # over time, doesn't need dims or data_order
	sigs_dict["betap"] = MdsSignal(r'\TOP:RESULTS:AEQDSK:betap', efit_tree)#, location=loc) # over time, doesn't need dims or data_order
	sigs_dict["betat"] = MdsSignal(r'\TOP:RESULTS:AEQDSK:betat', efit_tree)#, location=loc) # over time, doesn't need dims or data_order
	sigs_dict["tritop"] = MdsSignal(r'\TOP:RESULTS:AEQDSK:tritop', efit_tree)#, location=loc) # over time, doesn't need dims or data_order
	sigs_dict["tribot"] = MdsSignal(r'\TOP:RESULTS:AEQDSK:tribot', efit_tree)#, location=loc) # over time, doesn't need dims or data_order
	sigs_dict["s1"] = MdsSignal(r'\TOP:RESULTS:AEQDSK:s1', efit_tree)#, location=loc) # over time, doesn't need dims or data_order
	sigs_dict["s2"] = MdsSignal(r'\TOP:RESULTS:AEQDSK:s2', efit_tree)#, location=loc) # over time, doesn't need dims or data_order
	sigs_dict["s3"] = MdsSignal(r'\TOP:RESULTS:AEQDSK:s3', efit_tree)#, location=loc) # over time, doesn't need dims or data_order
	sigs_dict["ip"] = MdsSignal(r'\TOP:RESULTS:AEQDSK:ipmeas', efit_tree)#, location=loc) # over time, doesn't need dims or data_order
	sigs_dict["r0"] = MdsSignal(r'\TOP:RESULTS:AEQDSK:R0', efit_tree)#, location=loc) 
	sigs_dict["density"] = MdsSignal(r'\TOP:RESULTS:AEQDSK:DENSITY', efit_tree)#, location=loc) 
	sigs_dict["betan"] = MdsSignal(r'\TOP:RESULTS:AEQDSK:betan', efit_tree)#, location=loc) # over time, doesn't need dims or data_order
	...
	def calculate_geometric_and_profiles(record):
		...
		bcentr_val = np.abs(ds['bcentr']).values
		aminor_val = ds['aminor'].values
		...
		record['glitch'] = (s1/4 + (s2/4)*(1 + rcur)) - ds["li"].values/2 - ds["betap"].values
		record['shaf1'] = s1
		record['shaf2'] = s2
		...
		record['nwl_prox'] = ds['betan'].values/(4*ds['li'].values)
